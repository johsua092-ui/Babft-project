...
import { GEAR_TYPES } from "@/data/gears";

export default function GearsPage() {
  const [search, setSearch] = useState("");

  const filtered = GEAR_TYPES.filter(
    (g) => (g.name + " " + g.desc).toLowerCase().includes(search.toLowerCase())
  );
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center gap-3 py-20 animate-fade-in">
            ...
            <p style={{ color: "#8b949e" }}>Tidak ada gear yang cocok</p>
          </div>
        ) : (
            ...
            {filtered.map((gear, i) => (
              ...
              >
                <span
                  ...
                  style={{ background: gear.color, boxShadow: `0 0 10px ${gear.color}66` }}
                />
